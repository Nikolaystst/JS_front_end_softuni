function solve() {
    ...
    function onClick() {
        ...
        data = Object.entries(data).sort(([nameA, dataA], [nameB, dataB]) => {
            let salariesA = dataA.reduce((a, b) => {
                a.push(b.salary);
                return a
            }, []).reduce((a, b) => a + b) / dataA.length
            let salariesB = dataB.reduce((a, b) => {
                a.push(b.salary);
                return a
            }, []).reduce((a, b) => a + b) / dataB.length
            return salariesB - salariesA
        })

        const bestRestaurant = data[0]
        const salaries = bestRestaurant[1].reduce((a, b) => {
            a.push(b.salary);
            return a
        }, []).sort((a, b) => b - a)
